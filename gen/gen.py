...
def variant_type(oname, childrens):
  if childrens is not None:
    variants = childrens.get(oname, [])
    variants = [ '`' + snake_case(v) for v in variants + [ oname ]]
    return '[< %s ] gobject' % ' | '.join(variants)
  return '[ `%s ] gobject' % snake_case(oname)

# childrens is only used for args where covariance can be used.
def c_ml_type(type_, childrens=None):
  ...
  if t == 'interface':
    ...
    ml = variant_type(oname, childrens)
    return { 'c': t, 'ml': ml, 'base': False }

def handle_object_info(oinfo, childrens, f_c, f_ml, f_mli):
  ...
  for m in oinfo.get_methods():
    ...
    try:
      ...
      if m.is_method():
        c_type.append('t')
        ml_args.append('t__')
        ml_type.append(variant_type(oname, childrens))

      for arg in args:
        is_opt = arg.is_optional()
        arg_type = c_ml_type(arg.get_type(), childrens)
        if arg_type is None:
...
def write_files(f_c, f_ml, f_mli):
  ...
  for info in rep.get_infos('Arrow'):
    if isinstance(info, gi._gi.ObjectInfo):
      ...
      while True:
        pname = p.get_name()
        if pname not in childrens: childrens[pname] = []
        childrens[pname].append(oname)
        p = info.get_parent()
  ...
  for info in rep.get_infos('Arrow'):
    if isinstance(info, gi._gi.ObjectInfo) and not info.is_deprecated():
      handle_object_info(info, childrens, f_c, f_ml, f_mli)
  f_c.write('end')
